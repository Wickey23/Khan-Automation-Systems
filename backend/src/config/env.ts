import "dotenv/config";
import { z } from "zod";

const absoluteUrlSchema = z.string().url().refine((value) => {
  const protocol = new URL(value).protocol;
  return protocol === "http:" || protocol === "https:";
}, "Must be an absolute URL.");

const envSchema = z.object({
  PORT: z.string().default("4000"),
  DATABASE_URL: z.string().min(1),
  SECURITY_MODE: z.enum(["dev", "staging", "production"]).default("dev"),
  ALLOWED_ORIGIN: z.string().default("https://khansystems.com"),
  ALLOWED_ORIGINS: z.string().optional(),
  ALLOWED_ORIGIN_REGEX: z.string().optional(),
  JWT_SECRET: z.string().min(16),
  REFRESH_TOKEN_SECRET: z.string().optional(),
  ACCESS_TOKEN_TTL_MINUTES: z.string().default("15"),
  REFRESH_TOKEN_TTL_DAYS: z.string().default("14"),
  AUTH_SESSION_PERSISTENCE_ENABLED: z.string().default("true"),
  PASSWORD_RESET_TTL_MINUTES: z.string().default("30"),
  JWT_EXPIRES_IN: z.string().default("7d"),
  SMTP_HOST: z.string().optional(),
  SMTP_PORT: z.string().default("587"),
  SMTP_SECURE: z.string().default("false"),
  SMTP_USER: z.string().optional(),
  SMTP_PASS: z.string().optional(),
  SMTP_FROM: z.string().default("Khan Automation Systems <no-reply@example.com>"),
  RESEND_API_KEY: z.string().optional(),
  RESEND_FROM: z.string().optional(),
  EMAIL_FROM_PRODUCT: z.string().default("Khan Systems <hello@mail.khansystems.com>"),
  EMAIL_FROM_ALERTS: z.string().default("Khan Systems <alerts@mail.khansystems.com>"),
  EMAIL_FROM_OUTREACH: z.string().default("Sameer Khan <sameer@mail.khansystems.com>"),
  OUTREACH_BASE_URL: absoluteUrlSchema.default("https://khansystems.com"),
  LEAD_NOTIFICATION_EMAIL: z.string().email(),
  ADMIN_EMAIL: z.string().email(),
  ADMIN_PASSWORD: z.string().min(8),
  ADMIN_ACTION_PASSWORD: z.string().min(8),
  STRIPE_SECRET_KEY: z.string().default("sk_test_placeholder"),
  STRIPE_WEBHOOK_SECRET: z.string().default("whsec_placeholder"),
  STRIPE_STARTER_PRICE_ID: z.string().default("price_starter_placeholder"),
  STRIPE_PRO_PRICE_ID: z.string().default("price_pro_placeholder"),
  STRIPE_PRO_SEAT_PRICE_ID: z.string().optional(),
  STRIPE_FOUNDING_PRICE_ID: z.string().optional(),
  STRIPE_STARTER_SETUP_FEE_PRICE_ID: z.string().optional(),
  STRIPE_PRO_SETUP_FEE_PRICE_ID: z.string().optional(),
  STRIPE_FOUNDING_SETUP_FEE_PRICE_ID: z.string().optional(),
  STRIPE_SUCCESS_URL: absoluteUrlSchema.default("https://khansystems.com/checkout/success"),
  STRIPE_CANCEL_URL: absoluteUrlSchema.default("https://khansystems.com/checkout/cancel"),
  STRIPE_PORTAL_RETURN_URL: absoluteUrlSchema.optional(),
  FRONTEND_APP_URL: absoluteUrlSchema.default("https://khansystems.com"),
  API_BASE_URL: absoluteUrlSchema.default("https://api.khansystems.com"),
  AUTO_LIVE_ON_SETUP: z.string().default("false"),
  TWILIO_ACCOUNT_SID: z.string().optional(),
  TWILIO_AUTH_TOKEN: z.string().optional(),
  TWILIO_PHONE_SID: z.string().optional(),
  TWILIO_MESSAGING_SERVICE_SID: z.string().optional(),
  TWILIO_WEBHOOK_BASE_URL: absoluteUrlSchema.optional(),
  TWILIO_STATUS_CALLBACK_URL: absoluteUrlSchema.optional(),
  TWILIO_VALIDATE_SIGNATURES: z.string().default("true"),
  DEFAULT_VOICE_RING_TIMEOUT_SECONDS: z.string().default("20"),
  TWILIO_MEDIA_STREAM_BASE_URL: z.string().optional(),
  TWILIO_MEDIA_STREAM_STATUS_CALLBACK_URL: absoluteUrlSchema.optional(),
  TWILIO_MEDIA_STREAM_PATH: z.string().default("/ws/twilio/voice-media"),
  TWILIO_MEDIA_STREAM_TOKEN_SECRET: z.string().optional(),
  TWILIO_MEDIA_STREAM_ENABLED_DEFAULT: z.string().default("false"),
  DEEPGRAM_API_KEY: z.string().optional(),
  TRANSCRIPTION_PROVIDER: z.string().default("deepgram"),
  TRANSCRIPTION_ENABLED_DEFAULT: z.string().default("false"),
  CALL_SUMMARY_MODEL: z.string().default("gpt-4o-mini"),
  VAPI_API_KEY: z.string().optional(),
  VAPI_PRIVATE_KEY: z.string().optional(),
  VAPI_TOOL_SECRET: z.string().optional(),
  DEMO_VAPI_TOOL_SECRET: z.string().optional(),
  WEBHOOK_STRICT_MODE: z.string().default("true"),
  LOG_REDACTION_ENABLED: z.string().default("true"),
  ENCRYPTION_KEY_BASE64: z.string().optional(),
  GOOGLE_OAUTH_CLIENT_ID: z.string().optional(),
  GOOGLE_OAUTH_CLIENT_SECRET: z.string().optional(),
  GOOGLE_OAUTH_CALLBACK_URL: z.string().optional(),
  OUTLOOK_OAUTH_CLIENT_ID: z.string().optional(),
  OUTLOOK_OAUTH_CLIENT_SECRET: z.string().optional(),
  OUTLOOK_OAUTH_CALLBACK_URL: z.string().optional(),
  OPENAI_API_KEY: z.string().optional(),
  FEATURE_AI_OPS_ENABLED: z.string().default("false"),
  FEATURE_AI_AGENT_RUNS_ENABLED: z.string().default("false"),
  FEATURE_AI_APPROVALS_ENABLED: z.string().default("false"),
  FEATURE_AI_COPILOT_UI_ENABLED: z.string().default("false"),
  FEATURE_AI_MANAGER_INSIGHTS_ENABLED: z.string().default("false"),
  AI_OPS_DEFAULT_MODEL: z.string().default("gpt-4o-mini"),
  AI_OPS_MAX_TOOL_STEPS: z.string().default("6"),
  AI_OPS_RUN_TIMEOUT_MS: z.string().default("20000"),
  ADMIN_2FA_REQUIRED_IN_PROD: z.string().default("true"),
  AUTH_2FA_ENFORCE_ALL_USERS: z.string().default("false"),
  AUTH_2FA_TRUST_DAYS: z.string().default("1"),
  AUTH_STEP_UP_WINDOW_MINUTES: z.string().default("15"),
  VAPI_BACKFILL_ENABLED: z.string().default("true"),
  VAPI_BACKFILL_INTERVAL_MS: z.string().default("300000"),
  ROUTING_ENGINE_ENABLED: z.string().default("false"),
  AUTO_RECOVERY_ENABLED: z.string().default("false"),
  SLA_MONITOR_ENABLED: z.string().default("false"),
  DATA_INTEGRITY_GUARD_ENABLED: z.string().default("true"),
  FINALIZE_BOOKING_WORKER_INTERVAL_MS: z.string().default("30000"),
  AUTO_RECOVERY_DEDUPE_WINDOW_HOURS: z.string().default("2"),
  AUTO_RECOVERY_DAILY_CAP: z.string().default("50"),
  CALL_QUALITY_MIN_SCORE: z.string().default("75"),
  SLA_MONITOR_INTERVAL_MS: z.string().default("60000"),
  DATA_INTEGRITY_GUARD_INTERVAL_MS: z.string().default("3600000"),
  OUTREACH_RUNNER_ENABLED: z.string().default("true"),
  OUTREACH_RUNNER_INTERVAL_MS: z.string().default("300000"),
  OUTREACH_PROCESSING_TIMEOUT_MS: z.string().default("900000"),
  OUTREACH_DAILY_SEND_CAP: z.string().default("40"),
  OUTREACH_SEND_WINDOW_START_HOUR: z.string().default("9"),
  OUTREACH_SEND_WINDOW_END_HOUR: z.string().default("17"),
  OUTREACH_SEND_JITTER_MINUTES: z.string().default("20"),
  ADMIN_REPORTS_ENABLED: z.string().default("true"),
  ADMIN_REPORTS_RUNNER_INTERVAL_MS: z.string().default("900000"),
  ADMIN_REPORTS_DAILY_HOUR_UTC: z.string().default("13"),
  ADMIN_REPORTS_WEEKLY_DAY_UTC: z.string().default("1"),
  ADMIN_REPORTS_WEEKLY_HOUR_UTC: z.string().default("13"),
  SLA_WARN_WEBHOOK_FAILURES: z.string().default("5"),
  SLA_CRITICAL_WEBHOOK_FAILURES: z.string().default("10"),
  SLA_WARN_PROVIDER_ERRORS: z.string().default("5"),
  SLA_CRITICAL_PROVIDER_ERRORS: z.string().default("10"),
  SLA_CRITICAL_CONSECUTIVE_BREACHES: z.string().default("2"),
  SLA_RECOVERY_WINDOWS_REQUIRED: z.string().default("2"),
  OPS_ORG_EXPOSURE_THRESHOLD: z.string().default("0.5"),
  OPS_TRAFFIC_EXPOSURE_THRESHOLD: z.string().default("0.5"),
  GUIDED_DEMO_ENABLED: z.string().default("true"),
  GUIDED_DEMO_WINDOW_DAYS: z.string().default("7"),
  GUIDED_DEMO_TOTAL_CALL_CAP: z.string().default("15"),
  GUIDED_DEMO_UPGRADE_SMS_COOLDOWN_HOURS: z.string().default("24"),
  GUIDED_DEMO_UPGRADE_SMS_TEMPLATE: z.string().default(
    "Thanks for calling {{businessName}}. Your guided demo call limit has been reached. Reply here or upgrade in your billing portal to continue AI call handling."
  ),
  FEATURE_APPOINTMENTS_ENABLED: z.string().default("false"),
  FEATURE_CALENDAR_OAUTH_ENABLED: z.string().default("false"),
  FEATURE_NOTIFICATIONS_V1_ENABLED: z.string().default("false"),
  FEATURE_CLASSIFICATION_V1_ENABLED: z.string().default("false"),
  FEATURE_PIPELINE_STAGE_ENABLED: z.string().default("false"),
  FEATURE_OUTREACH_ENABLED: z.string().default("false"),
  FEATURE_PHASE1_ORG_ALLOWLIST: z.string().default(""),
  WEBHOOK_RATE_LIMIT_WINDOW_MS: z.string().default("60000"),
  WEBHOOK_RATE_LIMIT_MAX: z.string().default("600"),
  TOOL_RATE_LIMIT_WINDOW_MS: z.string().default("60000"),
  TOOL_RATE_LIMIT_MAX: z.string().default("300"),
  SMS_ORG_HOURLY_CAP: z.string().default("60"),
  SMS_ORG_DAILY_CAP: z.string().default("250"),
  APPOINTMENT_REQUEST_SLOT_OFFER_MAX: z.string().default("4"),
  APPOINTMENT_REQUEST_CLARIFICATION_MAX: z.string().default("3")
});

const parsed = envSchema.safeParse(process.env);
if (!parsed.success) {
  // eslint-disable-next-line no-console
  console.error(parsed.error.flatten().fieldErrors);
  throw new Error("Invalid environment configuration.");
}

if (parsed.data.SECURITY_MODE === "production") {
  const httpsFields = [
    "STRIPE_SUCCESS_URL",
    "STRIPE_CANCEL_URL",
    "STRIPE_PORTAL_RETURN_URL",
    "FRONTEND_APP_URL",
    "API_BASE_URL"
  ] as const;
  for (const key of httpsFields) {
    const value = parsed.data[key];
    if (!value) continue;
    if (!String(value).startsWith("https://")) {
      throw new Error(`Invalid environment configuration: ${key} must use https in production.`);
    }
  }
}

export const env = {
  ...parsed.data,
  // Accept either name so existing deployments using VAPI_PRIVATE_KEY keep working.
  VAPI_API_KEY: parsed.data.VAPI_API_KEY || parsed.data.VAPI_PRIVATE_KEY
};
