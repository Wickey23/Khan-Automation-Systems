...
stripeRouter.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    return res.status(401).send(`Webhook Error: ${error instanceof Error ? error.message : "unknown"}`);
  }
  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (customerEmail && customerId && subscriptionId) {
        ...
        if (!client) return res.status(200).json({ received: true, processed: false });
    ...
    await prisma.auditLog
      .create({
        data: {
          actorUserId: "stripe-webhook",
          actorRole: "SYSTEM",
          action: "STRIPE_WEBHOOK_PROCESSING_ERROR_IGNORED",
          metadataJson: JSON.stringify({
            eventType: event.type,
            eventId: event.id,
