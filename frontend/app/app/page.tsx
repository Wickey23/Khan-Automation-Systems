  ...
  return (
    <Link
      ...
      className={cn(
        ...
        priorityClasses,
        spotlight ? "ring-1 ring-slate-200" : "",
        order === "primary" ? "bg-white" : "opacity-95"
      )}
    >
      {priority === "critical" ? <span className="absolute left-0 top-0 h-full w-1 rounded-l-xl bg-rose-400" /> : null}
  ...
  return (
    <div className="space-y-2.5 pb-6">
      <CommandHeader
        eyebrow="Workspace"
        title="Dashboard"
        description="Good morning, Operator."
        actions={
          <div className="flex items-center gap-2">
        ...
        <>
          <section className={cn("grid gap-1.5", kpiCards.length >= 4 ? "md:grid-cols-2 xl:grid-cols-4" : "md:grid-cols-2 xl:grid-cols-3")}>
            {kpiCards.map((card) => (
              <div
                key={card.key}
                className="rounded-xl border border-slate-200 bg-white px-5 py-4 transition-colors duration-150 hover:border-slate-300"
              >
                <p className="text-xs font-semibold text-slate-500">{card.label}</p>
                <p className={cn("mt-1 text-3xl font-bold tracking-tight", card.emphasize ? "text-rose-700" : "text-slate-900")}>{card.value}</p>
            ...
            <section className="xl:col-span-8 rounded-xl border border-slate-200 bg-white">
              <div className="flex items-center justify-between border-b border-slate-200 px-4 py-2.5 sm:px-5">
                <div>
                  ...
                  <h2 className="text-lg font-bold tracking-tight text-slate-900">Action queue</h2>
                </div>
                  ...
                  <Link
                    ...
                    className={cn(
                      ...
                      topTask.priority === "critical"
                        ? "border-rose-200 bg-white ring-1 ring-slate-200"
                        : "border-slate-200 bg-white"
                    )}
                  >
                  ...
                  <div className="mt-2 space-y-2">
                    {readinessOpenItems.map((item) => (
                      <Link
                        key={item.key}
                        href={item.href}
                        className="group block rounded-md border border-slate-200 bg-slate-50 px-2.5 py-2 transition-all duration-150 hover:border-slate-300 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-300"
                      >
