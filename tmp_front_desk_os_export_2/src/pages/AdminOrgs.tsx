          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50 text-[11px] font-bold uppercase tracking-wider text-slate-500 border-b border-slate-200">
                ...
                <th className="px-8 py-4">Plan / Billing</th>
                <th className="px-8 py-4">Usage (MTD)</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {[
                { id: 'org_123', name: 'Acme Dental', domain: 'acmedental.com', plan: 'Enterprise', billing: 'Monthly', usage: '4,284 mins', status: 'Active', color: 'bg-primary/10 text-primary' },
                { id: 'org_456', name: 'Global Real Estate', domain: 'globalre.io', plan: 'Professional', billing: 'Annual', usage: '1,120 mins', status: 'Active', color: 'bg-emerald-100 text-emerald-600' },
                { id: 'org_789', name: 'Tech Solutions', domain: 'techsol.net', plan: 'Starter', billing: 'Monthly', usage: '450 mins', status: 'Trialing', color: 'bg-amber-100 text-amber-600' },
                { id: 'org_012', name: 'Legacy Corp', domain: 'legacy.com', plan: 'Enterprise', billing: 'Monthly', usage: '0 mins', status: 'Suspended', color: 'bg-red-100 text-red-600' },
              ].map((org, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors group">
                  <td className="px-8 py-5">
                    <Link to={`/admin/orgs/${org.id}`} className="flex items-center gap-4">
                      <div className={cn("h-12 w-12 rounded-2xl flex items-center justify-center font-bold text-lg shadow-sm", org.color)}>
                        {org.name[0]}
                      </div>
                      <div>
                  ...
                  <td className="px-8 py-5">
                    <div>
                      <span className="text-sm font-bold text-slate-700">{org.plan}</span>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">{org.billing}</p>
                    </div>
                  </td>
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-2">
                      <Activity size={14} className="text-slate-400" />
                      <span className="text-sm font-medium text-slate-600">{org.usage}</span>
                    </div>
                  </td>
                  <td className="px-8 py-5">
                    <span className={cn(
